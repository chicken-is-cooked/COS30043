...
Vue.createApp({
  ...
  computed: {
    ...
    filterUnits() {
      const codeKey = (this.UnitObj.code || '').toLowerCase();
      const typeKey = (this.UnitObj.type || ''); 

      return this.UnitInfor.filter(u =>
        u.code.toLowerCase().match(codeKey) &&
        (typeKey === 'All' || u.type.toLowerCase().match(typeKey.toLowerCase()))
      );
    }
  }
}).mount('#app');
